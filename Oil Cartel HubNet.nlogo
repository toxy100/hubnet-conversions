...
sellers-own [
  ...
  strategy-of-seller                        ;; set by client.  explained in "Make Offer" procedure
  seller-extra-output                    ;; set by client.  part of "Quota-Plus" strategy. explained in "Make Offer" procedure
  seller-reduced-output                  ;; set by client.  part of "Quota-Minus" strategy. explained in "Make Offer" procedure
]
...
to make-offers
  ask sellers [
    ...
    ifelse strategy-of-seller = "Agreement"
    ...
    [ ifelse strategy-of-seller = "Quota-Plus"
      [ set price-1 monopoly-price
        set p1-quantity-offered individual-quota
        set price-2 (monopoly-price - 100)
        set p2-quantity-offered seller-extra-output
       ]

      ;; "Quota-Minus" Strategy:  Consistently produce "reduced-output" amt below the quota
      ;; in an effort to keep prices up
      [ ifelse strategy-of-seller = "Quota-Minus"
        [ set price-1 monopoly-price
          set p1-quantity-offered (individual-quota - seller-reduced-output)
          set price-2 monopoly-price
          set p2-quantity-offered 0
        ]

        ;; "Flood Market" Strategy:  Saturate the market with low cost oil to punish cheaters.
        [ ifelse strategy-of-seller = "Flood Market"
          [  set price-1 monopoly-price
